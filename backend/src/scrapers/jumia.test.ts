...
describe('JumiaScraper', () => {
  ...
  describe('platformName', () => {
    test('returns correct platform name', () => {
      expect(scraper.platformName).toBe('jumia');
    });
  });

  describe('isValidUrl', () => {
    test('accepts valid Jumia URLs with www', () => {
      expect(scraper.isValidUrl('https://www.jumia.com.ng/product-123/')).toBe(true);
    });

    test('accepts valid Jumia URLs without www', () => {
      expect(scraper.isValidUrl('https://jumia.com.ng/product-123/')).toBe(true);
    });

    test('rejects non-Jumia URLs', () => {
      expect(scraper.isValidUrl('https://www.konga.com/product-123/')).toBe(false);
